...
help:
	...
	@echo "  PM2 相关命令（当前服务器暂时停用，建议使用 nohup 相关命令）"
	@echo "  make deploy BACKEND_URL=<url>    一键安装依赖、构建前端、初始化后端并用 PM2 启动（暂时停用）"
	@echo "  make start BACKEND_URL=<url>     构建并启动前后端 PM2 服务（暂时停用）"
	@echo "  make update BACKEND_URL=<url>    拉取代码、重建前端、同步后端依赖并重启（暂时停用）"
	@echo "  make stop                        停止 PM2 服务（暂时停用）"
	@echo "  make restart                     重启 PM2 服务（暂时停用）"
	@echo "  make status                      查看 PM2 状态（暂时停用）"
	@echo "  make logs                        查看 PM2 日志（暂时停用）"
	@echo "  make backend-logs                实时查看后端应用日志（暂时停用）"
...
sync-backend:
	@bash -lc '. ./scripts/deploy_common.sh && run_project_step "同步后端依赖" "$$BACKEND_DIR" "uv sync --frozen"'
